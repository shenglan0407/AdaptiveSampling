...
# All rights reserved.
# 
# This script takes simulation data of the b2AR-alprenolol binding 
# generate the position of ligands for restarting  the next
...
sim_path = '/scratch/PI/rondror/MD_simulations/amber/b2AR_ligand_binding/alprenolol/ten_ligands/production/ten_ligands/aligned_trajs/'
...
for this_file in os.listdir(sim_path):
    if this_file.endswith('.nc'):
        sim_files.append(sim_path+this_file)
        
test_traj = md.load_frame(sim_files[0],0,top = topology)
# track atoms in ligands
ligands = ['ALP%d'%(ii+1) for ii in range(10)]

ind_ligs = [] 
for ligand in ligands:
    iis = [residue.index for residue in test_traj.topology.residues if str(residue)==ligand]
    ind_ligs.extend(iis)
print ind_ligs
...
ind_res = []
target_res = []
for residue in test_traj.topology.residues:
    if str(residue) in res_to_track:
        if residue.index in ind_res:
            pass
        else:
            ind_res.append(residue.index)
